...
window.addEventListener('keydown', event => {
  ...
  if (event.target instanceof HTMLElement && event.target.closest('select, input, textarea, button, dialog')) return;
  event.preventDefault();
  if (event.repeat) return;
  if (event.code === 'Escape') { if (started && match.phase !== 'ended') setPaused(!paused); return; }
  press(event.code);
});
...
class PitchScene extends Phaser.Scene {
  ...
  create() {
    ...
    for (const p of match.players) this.marks.push(this.add.text(p.x, p.y, p.keeper ? '1' : p.id % 3 === 1 ? '7' : '9', { fontFamily: 'Arial', fontSize: '10px', fontStyle: 'bold', color: p.team === 0 ? '#183e38' : '#fff5e4' }).setOrigin(.5));
    this.draw();
  }
...
function startMatch() {
  unlockAudio(); match.start(); started = true; setPaused(false); sound('start');
...
function updateHud() {
  ...
  overlay.hidden = started && !paused && match.phase === 'playing';
